...
*					ejemplo:
*							 Etiqueta ant1= argumento1.get(r.getArgumento1());
...
//TODO CODIGO AQUI
		for (int n=0;n<reglas.size();n++)
	    {
			...
			{
				contador=0;
				logger(nombre+"Logger.csv",r,emparejamiento);
			}
...
//TODO CODIGO AQUI
    ...
    {       
		float hij;
		if(ej<e.getX0()) hij=0;
		else if(e.getX0()<ej && ej<e.getX1()) hij=(ej-e.getX0())/(e.getX1()-e.getX0());
		else if(e.getX1()<ej && ej<e.getX2()) hij=1;
		else hij=0;
		
		return  hij;// quitar luego, debe devolver elgrado de emparejamiento adecuado
    }
    ...
    /**
     * En base a la liste de emparejamientos y los puntos de corte genera una salida en el universo de discurso de 
